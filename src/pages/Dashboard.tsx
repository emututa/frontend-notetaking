...
import { useState, useEffect } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { Plus, Search, Heart, Briefcase, Lock, Star, FileText, Home, X, Menu } from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div className={`w-64 bg-white border-r border-gray-200 flex flex-col fixed lg:relative h-full z-30 transform transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        {/* Logo */}
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center gap-2 mb-1">
            <FileText className="w-6 h-6 text-blue-600" />
        ...
        <header className="bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-8 py-4 lg:py-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Write smart</h1>
                <p className="text-gray-500 mt-1 text-sm">It's time to note it down</p>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  value={searchQuery}
                  onChange={handleSearchChange}
                  className="pl-10 pr-10 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent w-32 sm:w-48 md:w-64"
                />
                {searchQuery && (
                  <button 
                    onClick={clearSearch}
              ...
              <Link to="/notes/new" className="hidden sm:block">
                <button className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg py-2 px-4 flex items-center gap-2 transition-colors">
                  <Plus className="w-4 h-4" />
                  <span className="font-medium">New Note</span>
                </button>
              </Link>
              <Link to="/notes/new" className="sm:hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 lg:mb-8">
              {categoryStats.map((category) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br ${category.gradient} flex items-center justify-center mb-3 sm:mb-4`}>
                    ...
                    <p className="text-xs sm:text-sm text-gray-500 mt-1">notes</p>
                  </Link>
                );
              })}
            </div>
          ...
          <div>
            {isSearching ? (
              ...
              <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-4">Recent Notes</h2>
            )}
            ...
            {filteredNotes.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {filteredNotes.map((note) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="font-semibold text-gray-900 line-clamp-1 text-sm sm:text-base">{note.title}</h3>
                      <span className={`text-xs px-2 py-1 rounded-full bg-gradient-to-br ${
                        categoryStats.find(c => c.name.toLowerCase() === note.category)?.gradient || 'from-gray-400 to-gray-500'
                    ...
                    <p className="text-gray-600 text-xs sm:text-sm line-clamp-3">{note.content}</p>
              ...
              <div className="bg-white rounded-xl p-8 sm:p-12 border border-gray-200 text-center">
                ...
                <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-2">No notes found</h3>
                <p className="text-sm sm:text-base text-gray-500 mb-4 sm:mb-6">Try searching with different keywords</p>
                <button 
                  onClick={clearSearch}
              ...
              <div className="bg-white rounded-xl p-8 sm:p-12 border border-gray-200 text-center">
                ...
                <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-2">No notes yet</h3>
                <p className="text-sm sm:text-base text-gray-500 mb-4 sm:mb-6">Start creating your first note to see it here</p>
                <Link to="/create">
                  <button className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg py-2 px-4 sm:px-6 inline-flex items-center gap-2 transition-colors text-sm sm:text-base">
                    <Plus className="w-4 h-4" />
                    <span className="font-medium">Create Note</span>
                  </button>
                </Link>
